...
function Sidebar({ isOpen }) {
    ...
    const navItems = [
        ...
        ...(user && Number(user.rol_id) === 1 ? [{ path: "/movimientos", label: "Movimientos", icon: "bi-arrow-left-right" }] : []),
        { path: "/equipos", label: "Equipos", icon: "bi-tools" },
    ];

    return (
        <aside 
            className={`w-64 h-screen fixed left-0 top-0 z-50 flex flex-col border-r transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.path}
                            to={item.path}
                            className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all no-underline hover:no-underline ${
                                active
            ...
            <div className="p-4 border-t flex flex-col gap-3" style={{ backgroundColor: 'var(--bg-primary)', borderColor: 'var(--border-color)' }}>
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="text-sm font-bold truncate" style={{ color: 'var(--text-primary)' }}>{user?.nombre || 'Usuario'}</div>
                        <div className="text-xs truncate" style={{ color: 'var(--text-secondary)' }}>{user?.rol_id === 1 ? 'Administrador' : 'Empleado'}</div>
                    </div>
                </div>
